  ...
  return (
    <div className="min-h-screen bg-[#0A1628] flex flex-col px-6 pt-14 pb-10 font-sans">
      ...
      <div className="mb-6">
        ...
        <p className="text-white/50 text-sm">Based on your answers, here's how MasterGrowbot helps you specifically.</p>
      </div>

      {/* Personalization Card */}
      <div className={`bg-[#059669]/10 border border-[#059669]/30 rounded-2xl p-4 mb-5 transition-all duration-500 ${visible ? 'opacity-100' : 'opacity-0'}`}>
        <div className="text-[#059669] text-xs font-black uppercase tracking-widest mb-2">Your Profile</div>
        <div className="text-white text-sm font-semibold mb-1">{EXP_MESSAGES[experienceLevel] || "We'll personalize guidance for your level."}</div>
        <div className="text-white/60 text-xs">{GOAL_MESSAGES[goal] || "We'll tailor your experience to your goals."}</div>
      </div>
      ...
      <div className="flex-1 space-y-3">
        {relevantSolutions.length > 0 ? (
          <>
            <div className="text-white/40 text-xs font-bold uppercase tracking-widest mb-1">Solutions built for your challenges:</div>
            {relevantSolutions.map((sol, i) => (
              <div
                key={i}
                className={`flex items-start gap-4 bg-white/5 border border-white/10 rounded-2xl p-4 transition-all duration-500 ${
                  step > i ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'
                }`}
              >
                <span className="text-2xl leading-none mt-0.5">{sol.icon}</span>
                <div className="flex-1">
                  <div className="text-white font-black text-sm mb-1">{sol.title}</div>
                  <div className="text-white/50 text-xs leading-relaxed">{sol.solution}</div>
                </div>
          ...
          // No specific pain points selected — show generic benefits
          <div className="space-y-3">
            {[
              ...
              <div key={i} className={`flex items-start gap-4 bg-white/5 border border-white/10 rounded-2xl p-4 transition-all duration-500 ${step > i ? 'opacity-100' : 'opacity-0'}`} style={{ transitionDelay: `${i * 150}ms` }}>
