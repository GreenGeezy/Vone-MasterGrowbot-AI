...
import { Send, AudioLines, X, Power, Settings2, Check, Activity } from 'lucide-react';
import { sendMessage } from '../services/geminiService';
import { ChatMessage, JournalEntry, Plant, UserProfile } from '../types';
import Growbot from '../components/Growbot';

interface ChatProps { onSaveToJournal?: (entry: any) => void; plant?: Plant; userProfile?: UserProfile | null; }

const SUGGESTED_PROMPTS = ["Fix yellow leaves", "Feeding schedule", "Harvest check", "Mold prevention"];
const VOICE_OPTIONS = [
  { id: 'Kore', label: 'Coach Kore', type: 'Calm' }, { id: 'Charon', label: 'Coach Mike', type: 'Bold' },
  { id: 'Fenrir', label: 'MasterGrowbot', type: 'Synthetic' }, { id: 'Puck', label: 'Coach Puck', type: 'Energetic' },
];

const Chat: React.FC<ChatProps> = ({ onSaveToJournal, plant, userProfile }) => {
  const [messages, setMessages] = useState<ChatMessage[]>([{ id: '1', text: "Greetings. I am MasterGrowbot. How is your garden thriving today?", role: 'model' }]);
  ...
  const handleSend = async (text: string) => {
    if (!text.trim()) return;
    setMessages(p => [...p, { id: Date.now().toString(), text, role: 'user' }]);
    setInput('');
    setLoading(true);
    try {
        const response = await sendMessage(text, plant, userProfile, selectedVoice);
        setMessages(p => [...p, { id: Date.now().toString(), text: response, role: 'model' }]);
    ...
    <div className="flex flex-col h-full bg-surface pb-24 relative overflow-hidden font-sans">
      <div className="bg-white/90 backdrop-blur border-b border-gray-100 p-4 pt-6 flex items-center justify-between sticky top-0 z-20">
        ...
        <div className="relative">
            <button onClick={() => setShowVoiceSettings(!showVoiceSettings)} className="p-2 text-text-sub hover:text-primary bg-gray-50 rounded-full"><Settings2 size={20} /></button>
            {showVoiceSettings && (
                <div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-xl shadow-xl p-2 z-50">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 z-10">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`p-4 text-sm max-w-[85%] rounded-2xl shadow-sm ${msg.role === 'user' ? 'bg-text-main text-white rounded-tr-sm' : 'bg-white text-text-main rounded-tl-sm border border-gray-100'}`}>{msg.text}</div>
          </div>
        ))}
        {loading && <div className="pl-4 text-xs text-gray-400">Thinking...</div>}
        <div ref={endRef} />
      </div>
      <div className="p-4 bg-white/80 backdrop-blur-lg border-t border-gray-100 z-30 pb-6">
        <div className="flex overflow-x-auto gap-2 mb-3 no-scrollbar">
            {SUGGESTED_PROMPTS.map(p => <button key={p} onClick={() => handleSend(p)} className="px-4 py-2 bg-white border border-gray-200 rounded-full text-xs font-bold shadow-sm">{p}</button>)}
        </div>
        <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-[24px] px-2 py-2">
          ...
          <button onClick={() => {setIsLive(true); setTimeout(()=>setLiveTranscript("Listening..."),1500)}} className="p-2 rounded-full text-text-sub hover:text-red-500"><AudioLines size={20} /></button>
