...
interface PaywallProps {
  ...
  onSkip?: () => void; 
  onPurchase?: () => void; // Added to match App.tsx prop
  isMandatory?: boolean;
...
const Paywall: React.FC<PaywallProps> = ({ onClose, onAuthRedirect, onSkip, onPurchase, isMandatory = false, userProfile }) => {
  ...
  const handleStartTrial = async () => {
    ...
    try {
        if (!Capacitor.isNativePlatform()) {
            ...
            // Web Success Logic
            if (onPurchase) onPurchase(); // Notify App.tsx
            else if (onAuthRedirect && !session) onAuthRedirect();
            else onClose();
            return;
        } 
        ...
        if (offerings.current && offerings.current.availablePackages.length > 0) {
             ...
             // Proceed
             if (onPurchase) onPurchase(); // Notify App.tsx to unlock features
             else if (onAuthRedirect && !session) onAuthRedirect();
             else onClose();

        } else {
            throw new Error("No offerings available. Check Google Play Console setup.");
        }
