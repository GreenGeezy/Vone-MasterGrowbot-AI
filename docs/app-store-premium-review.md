...
## Current build: 1.6.8 (169)

See [release-1.6.8.md](release-1.6.8.md) for the expanded video report, journal-ready care checks, share caption CTA, and Premium paywall update. The existing photo analysis remains unchanged.
...
2. Add all three Premium subscriptions to the 1.6.8 app-version submission. Do not submit them separately before the matching binary is available.
...
4. Run the **MasterGrowbot iOS App Store** (`ios-app-store`) Codemagic workflow from branch `ios`. Test build 169 in Sandbox/TestFlight before submitting it for review.

## Products to add to version 1.6.8
...
On an actual iPhone running build 169:
...
> MasterGrowbot AI is intended only for lawful use where permitted by applicable local laws. It does not sell or deliver cannabis, provide consumption functionality, or provide medical claims. Version 1.6.8 expands general visual observations from user-supplied plant videos with confidence, priority inspection, journal-ready care checks, and grow-wide context. Results separate visible evidence from possible interpretations and do not provide controlled-substance cultivation optimization instructions.
>
...
5. Confirm TestFlight reports version **1.6.8 (169)**. If build 169 already exists, increment only the iOS build number, commit it, and rerun.
